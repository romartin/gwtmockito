 ...
 * {@link GwtMockitoTestRunner3} will behave as follows:
 *
 * <ul>
 ...
 *      {@link GwtMockitoTestRunner3#getClassesToStub}.
 * </ul>
 *
 * @see GwtMockito
 * @author ekuefler@google.com (Erik Kuefler)
 */
public class GwtMockitoTestRunner3 extends BlockJUnit4ClassRunner {
   ...
   */
  public GwtMockitoTestRunner3(Class<?> unitTestClass) throws InitializationError {
    super(unitTestClass);
   ...
   * extending {@link GwtMockitoTestRunner3} and overriding this method. This allows users to
   ...
   * runner extending {@link GwtMockitoTestRunner3} and overriding this method.
   *
  ...
  private final class GwtMockitoClassLoader extends Loader implements Translator {
    ...
    GwtMockitoClassLoader(ClassLoader classLoader, ClassPool classPool) {
      super(classLoader == null ? GwtMockitoTestRunner3.class.getClassLoader() : classLoader, classPool);
      try {
    ...
    public void onLoad(ClassPool pool, String name)
      ...
      if (!clazz.isFrozen()) {
        for (CtMethod method : clazz.getMethods()) {
          ...
          if (isShadeMethod) {
            isNode = isNodeClass(name);
            if (isNode) {
              method.setBody(String.format("{ return (%s) this; }", clazz.getName()));
              /*method.setBody(String.format(
                      "return (%s) com.google.gwtmockito.impl.StubGenerator.invokeCast("
                              + "Class.forName(\"%s\"), \"%s\", \"%s\");",
      ...
      for (CtMethod method : clazz.getDeclaredMethods()) {
        if (StubGenerator.shouldStub(method, getClassesToStub())) {
          ...
          boolean isShadeMethod = isNode && isCastMethod(method.getName());
          if ( !isShadeMethod ) {
    ...
    private boolean isNodeClass(String name) {
      return name != null && name.startsWith("com.ait.lienzo.client.core.shape") && !name.equals("com.ait.lienzo.client.core.shape.Node");
      /*if ( name != null && name.startsWith("com.ait.lienzo.client.core.shape") ) {
        try {
          Class clazz = Class.forName(name);
          Class superClass = clazz.getSuperclass();
          while ( superClass != null && !superClass.equals(Object.class) ) {
            superClass = superClass.getSuperclass();
            if ( "com.ait.lienzo.client.core.shape.Node".equals(superClass.getName()) ) {
              return true;
            }
          }
